...
-- ALL ACTIONS
CREATE VIEW consensus_2pc_actions_all AS
SELECT id,
       event_id,
       action_type,
       circuit_id,
...
-- ALL EVENTS
CREATE VIEW consensus_2pc_events_all AS
SELECT id,
       circuit_id,
       service_id,
       event_type,
...
LEFT JOIN consensus_2pc_start_event ON consensus_2pc_event.id=consensus_2pc_start_event.event_id
LEFT JOIN consensus_2pc_vote_event ON consensus_2pc_event.id=consensus_2pc_vote_event.event_id
ORDER BY id;

-- ALL ACTIONS AND EVENTS
CREATE VIEW consensus_2pc_actions_and_events_all AS 
SELECT id AS a_id,
       ...
       executed_at,
       NULL as e_id,
       NULL as e_event_type,
       NULL as d_epoch,
       NULL as d_receiver_service_id,
       NULL as d_message_type,
       NULL as d_vote_response,
       NULL as d_vote_request,
       NULL as s_value,
       NULL as v_vote,
       NULL as e_executed_epoch
FROM consensus_2pc_action
LEFT JOIN consensus_2pc_notification_action ON consensus_2pc_action.id=consensus_2pc_notification_action.action_id
LEFT JOIN consensus_2pc_send_message_action ON consensus_2pc_action.id=consensus_2pc_send_message_action.action_id
LEFT JOIN consensus_2pc_update_context_action ON consensus_2pc_action.id=consensus_2pc_update_context_action.action_id
...
SELECT NULL AS a_id,
       NULL AS a_event_id,
       NULL AS a_event_type,
       NULL as n_notification_type,
       NULL as n_dropped_message,
       NULL as n_request_for_vote_value,
       NULL as s_epoch,
       NULL as s_receiver_service_id,
       NULL as s_message_type,
       NULL as s_vote_response,
       NULL as s_vote_request,
       NULL as uc_coordinator,
       NULL as uc_epoch,
       NULL as uc_last_commit_epoch,
       NULL as uc_state,
       NULL as uc_vote_timeout_start,
       NULL as uc_vote,
       NULL as uc_decision_timeout_start,
       NULL as uc_action_alarm,
       NULL as uc_ack_timeout_start,
       NULL as ucp_process,
       NULL as ucp_vote,
       NULL as ucp_decision_ack,
       circuit_id,
       service_id,
       created_at,
       executed_at,

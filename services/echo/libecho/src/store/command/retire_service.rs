...
impl<C> StoreCommand for EchoRetireServiceCommand<C> {
    type Context = C;

    // uses the `update_service_status` store method to set the service status to
    // `EchoServiceStatus::Finalized`
    fn execute(&self, conn: &Self::Context) -> Result<(), InternalError> {
        self.store_factory
            .new_store(conn)
            .update_service_status(&self.service, EchoServiceStatus::Retired)
    }
}

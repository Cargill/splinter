...
Currently, OAuth is the only authorization type supported by splinterd. The REST
API requires that an authorization provider is configured, so OAuth must be setup
in order to start the REST API.
...
All 4 of the above arguments must be provided. If any argument is missing,
splinterd will fail to start.

...
fn main() {
    ...
    let log_handle = {
        ...
        {
            let default_config: config::LogConfig = Default::default();
            let log_handle = log4rs::init_config(
                //Always produces a valid config try_into_config just returns a Result
                default_config
                    .try_into_config()
                    .expect("Default logging config failure"),
            );
            if let Err(e) = log_handle {

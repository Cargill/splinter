...
impl AdminSubsystemBuilder {
    ...
    pub fn build(mut self) -> Result<RunnableAdminSubsystem, InternalError> {
        ...
        let scabbard_service_factory = self
            ...
            .map(|config| {
                {
                    let pool = config.connection_pool.write().map_err(|_| {
                        InternalError::with_message(
                            "Cannot build AdminSubsystem, connection pool lock poisoned".into(),
                        )
                    })?;
                ...
                ScabbardFactoryBuilder::new()
                    ...
                    .with_storage_configuration(config.connection_pool.into())

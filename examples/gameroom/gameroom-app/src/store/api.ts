...
export async function listNotifications(publicKey: string): Promise<GameroomNotification[]> {
  const isDisplayed = (value: GameroomNotification): boolean => {
    ...
    if (value.notification_type.includes('new_game_created')) {
      return true;
    }
    if (value.notification_type.includes('game_updated') && value.requester !== publicKey) {
      return true;
    }
    if (value.notification_type.includes('game_over')) {
      return true;
    }
